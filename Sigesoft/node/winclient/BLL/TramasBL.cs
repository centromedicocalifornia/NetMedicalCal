...
namespace Sigesoft.Node.WinClient.BLL
{
    public class TramasBL
    {
        ...
        public void ActualizarHospTrama(string hospId, int estado, string modo)
        {
            ...
            if (modo == "HOSP" && hospId != "")
            {
                ConexionSigesoft conectasam = new ConexionSigesoft();
                conectasam.opensigesoft();
                cadenaActualizar = "i_TramaHosp = " + estado;
                var cadena1 =
                    " UPDATE hospitalizacion SET " + cadenaActualizar +
            ...
            else if (modo == "SOP" && hospId != "")
            {
                ConexionSigesoft conectasam = new ConexionSigesoft();
                conectasam.opensigesoft();
                cadenaActualizar = "i_TramaSop = " + estado;
                var cadena1 =
                    " UPDATE hospitalizacion SET " + cadenaActualizar +
                    "  where v_HopitalizacionId = '" + hospId + "'";
                SqlCommand comando = new SqlCommand(cadena1, connection: conectasam.conectarsigesoft);
        ...
        public void ActualizarHospTramaServicio(string servicioId, int estado, string modo)
        {
            ...
            if (modo == "HOSP" )
            {
                ...
                var cadena1 =
                    " UPDATE hospitalizacion SET " + cadenaActualizar +
                    "  where v_HopitalizacionId =  (select top 1 v_HopitalizacionId from hospitalizacionservice where v_ServiceId = '" + servicioId + "')";
                SqlCommand comando = new SqlCommand(cadena1, connection: conectasam.conectarsigesoft);
                SqlDataReader lector1 = comando.ExecuteReader();
                lector1.Close();
                conectasam.closesigesoft();
